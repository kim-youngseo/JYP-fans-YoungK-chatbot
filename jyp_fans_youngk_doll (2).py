...
st.title("KE🦊와 마이데이🍀의 대화방")
st.markdown("마이데이🍀의 메시지를 입력하면 KE🦊가 따뜻하게 답해줘요! 💬")
...
if st.button("💌 KE에게 보내기") and user_input:
    # 팬 메시지 저장
...
for i, msg in enumerate(st.session_state.messages[1:], 1):  # system 제외
    ...
    speaker = "마이데이🍀" if is_user else "KE🦊"
    st.markdown(f"**{speaker}:** {msg['content']}")

    # 🔸 특정 키워드 입력 시 내가 정한 이미지 출력
    trigger_keywords = ["생일카페", "영케이 생카", "이벤트", "2024 생카"]
    if is_user and any(keyword in msg["content"].lower() for keyword in trigger_keywords):
        st.image("images/youngk_cafe.jpg", caption="🎉 YoungK 생일카페 포스터!", use_column_width=True)

    # 🔸 기존 KE 인형 키워드 대응
    if not is_user and any(k in st.session_state.messages[i - 1]["content"].lower() for k in ["영케이랑 같이 찍은 사진", "ke 사진", "사진 보여줘", "ke 보여줘"]):
               selected = random.choice([
            "https://imgur.com/k2pQJYS",
            "https://imgur.com/kNl5V3z",
            "https://imgur.com/pvdJg2D"
        ])
        st.image(selected, caption="📸 KE가 보낸 사진!", use_column_width=True)