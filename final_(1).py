...
if st.button("💌 KE에게 보내기") and user_input.strip():
    ...
    if intent == "일정질문":
        modified_input = f"[일정 관련 질문이야] {modified_input}"
    elif intent == "위로필요":
        modified_input = f"[위로가 필요한 메시지야] {modified_input}"

    st.session_state.messages.append({"role": "user", "content": modified_input})
    st.session_state.admin_log.append(user_input.strip())
    ...
    if send_image:
        ...
        try:
            ...
            if "여기서 마이데이랑 이야기하고 있어" in response_text:
                response_text = "내일? 마이데이랑 즐거운 하루 보낼 예정이지~ 너는 어떤 하루 보낼 거야?"
