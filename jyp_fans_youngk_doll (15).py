...
if st.button("💌 KE에게 보내기") and user_input.strip():
    ...
    # 📸 이미지 전송 키워드 포함 시 응답 미리 생성
    if send_image:
        response_text = "📸 마이데이, 내가 직접 찍은 사진이야! 예쁘게 봐줘~🦊"
    else:
        try:
...
for i, msg in enumerate(st.session_state.messages[1:], 1):  # system 제외
    ...
    # 🎁 이미지 첨부 시점
    if not is_user and send_image and msg["content"].startswith("📸 마이데이"):
