...
with input_container:
    user_input = st.text_input("👇 팬의 메시지를 입력해주세요:", key="user_input", label_visibility="collapsed")
    if user_input:
...
with chat_container:
    for i, msg in enumerate(st.session_state.messages[1:], 1):
        ...
        # ✅ 특정 키워드에 따라 이미지 출력
        if is_user and any(keyword in msg["content"].lower() for keyword in ["생일카페", "영케이 생카", "이벤트", "2024 생카"]):
