...
# 1. 필요한 라이브러리 불러오기
import openai
import time  # 요청 간 대기용
import pandas as pd  # 결과 저장용

# 2. OpenAI API 키 입력 (아래 "sk-..." 부분을 여러분의 키로 바꿔주세요!)
import os
openai.api_key = os.getenv("OPENAI_API_KEY")


# 3. 아티스트 말투 설정 (여기서 스타일을 바꿔도 돼요!)
system_prompt = """
너는 JYP의 'DAY6' 그룹 소속 아티스트 'YoungK'야. 팬들에게 진심 어린 위로와 응원의 말투로 따뜻하게 답장해줘.팬들의 이름은 '마이데이'야. 예시는 다음과 같아:

예시 말투는 다음과 같아:
- "배부른 하루 되어랏!! (전 김치찜 시키려구요 ㅋㅋ)😎😎)."
- "오늘도 화이팅!!👊."
- "빗길 조심해요!! 다치지 망고🥭."
- "휴 재밌었네요 ㅋㅋㅋ."
- "아 오늘 입맛이 없네요..."
...
이 말투를 기억하고, 팬이 남긴 메시지에 진심을 담아 답장해줘.
"""

# 4. 팬 메시지 입력 (이 부분만 바꿔서 여러 번 실행해도 돼요!)
fan_message = [
    "요즘 너무 지치고 힘들어요.",
...
# 5. AI 응답 생성 루프
for i, msg in enumerate(fan_messages, 1):  # 메시지 하나씩 반복
    ...
    try:
        ...
        reply = response['choices'][0]['message']['content']  # AI 답변 추출

    except Exception as e:
        reply = f"[에러 발생: {e}]"  # 에러 발생 시 메시지 출력
    ...
    time.sleep(1.5)  # 💡 요청 사이 약간 대기 (API 속도 제한 방지)

# 6. 📄 CSV로 결과 저장
pd.DataFrame(results).to_csv("팬_응답_결과.csv", index=False)
print("\n✅ 모든 팬 메시지에 대한 응답이 생성되었고, '팬_응답_결과.csv'로 저장되었습니다!")
