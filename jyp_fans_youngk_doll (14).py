...
for i, msg in enumerate(st.session_state.messages[1:], 1):  # system 제외
    ...
    # 🧸 인형 관련 키워드 포함 시 KE가 직접 사진을 보내는 듯한 연출
    if not is_user and any(
        ...
        photo_caption = "📸 마이데이, 내가 직접 찍은 사진이야! 예쁘게 봐줘~🦊"
        ...
        st.markdown(
            f"""
            <div style="background-color: #fff8e1; color: #000000; padding: 1rem;
                        border-radius: 12px; margin: 0.5rem 0; font-size: 16px;">
                <strong>🦊 KE:</strong><br>{photo_caption}
            </div>
            """,
            unsafe_allow_html=True
        )
