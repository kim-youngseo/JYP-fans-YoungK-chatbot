...
if st.button("💌 KE에게 보내기") and user_input.strip():
    ...
    # 이미지 요청 시 챗봇 메시지와 이미지 삽입
    if send_image:
        # KE 메시지 (사진처럼 보이게)
        photo_msg = "📸 마이데이, 내가 직접 찍은 사진이야! 예쁘게 봐줘~🦊"
        st.session_state.messages.append({"role": "assistant", "content": photo_msg})

        # 이미지 URL 삽입
        st.session_state.messages.append({"role": "image", "content": random.choice([
...
for msg in st.session_state.messages[1:]:  # system 제외
    ...
    if role == "user":
        ...
        st.image(msg["content"], caption="🧸 KE가 보낸 사진", use_container_width=True)
        continue
    else:
        continue
