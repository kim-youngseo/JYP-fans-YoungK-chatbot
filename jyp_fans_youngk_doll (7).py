...
# ✅ 유저 입력
user_input = st.text_input("👇 팬의 메시지를 입력해주세요:", key="user_input", on_change=lambda: None)
...
for i, msg in enumerate(st.session_state.messages[1:], 1):
    ...
    st.write(f"**{speaker}:** {msg['content']}")  # 🔄 st.write로 취소선 문제 해결

    # KE의 응답 직후, 유저 메시지에 이미지 키워드가 있는 경우
    if not is_user and any(
        ...
        st.image(selected, caption="📸 KE가 직접 보낸 사진!", use_container_width=True)